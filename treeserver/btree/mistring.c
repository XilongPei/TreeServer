 ...
 * string utility
 * single user version
 * Writen by Xilong Pei   Sep. 12  1989, 1998
 *
 **************************************************************************/
...
static char subStaticStr[NORMAL_SUBSTR_LEN];
...
unsigned char *csubstr(unsigned char *source_string, \
		       short start_pos, \
		       short length)
{
...
char *SubstrOutBlank(char *source_string, short start_pos, short length)
{
   ...
   substring = (char *)realloc(substring, length+1);
   for(i=0; i<length && i<source_len; i++)
       substring[i]=source_string[start_pos++];

   for(i--; i>=0 && substring[i]==' '; i--)   /* out blank */
	;
