				      ...
				      unsigned char *szKey )
{
    ...
    {
	if ( strnicmp( szGroup, csu->cacheMem + *(unsigned short *)(p+4),   \
    ...
    if ( p >= tail || *(p+1) != groupLen || *p != 0 )  return -3;

    if ( labelLen == 4 && keyLen == 4 )
	return *(long*)(csu->cacheMem + *(unsigned short *)(p+4) + *(p+1)-4);

    tail = p + *(unsigned short *)(p+2);
    p += 6;
    ...
    {
	if ( strnicmp( szLabel, csu->cacheMem + *(unsigned short *)(p+4),   \
	...
	while ( p < tail && *p == 2 )
	{
	    if ( strnicmp( szKey, csu->cacheMem + *(unsigned short *)(p+2), \
							  keyLen - 4 ) == 0 )
	    {
		break;
	    }
	...
	return *(long*)(csu->cacheMem + *(unsigned short *)(p+2) + *(p+1)-4);
...
short addGroupToCache( PCFG_STRU csu, unsigned char *szGroup, long offset )
{
    ...
    memmove( tableHead - 6, tableHead, moveLen );
    ...
    *(unsigned short *)(p+4) = csu->cacheMemTail;
...
short addLabelToCache( PCFG_STRU csu, unsigned char *szLabel, long offset )
{
    ...
    *(unsigned short *)(p+2) = 6;
    *(unsigned short *)(p+4) = csu->cacheMemTail;
    *(unsigned short *)(csu->cacheMem+csu->curGroupOffset+2) += 6;
...
short addKeyToCache( PCFG_STRU csu, unsigned char *szKey, long offset )
{
    ...
    memmove( tableHead - 4, tableHead, moveLen );
    csu->cacheTableHead -= 4;
    csu->curKeyOffset -= 4;
    csu->curLabelOffset -= 4;
    csu->curGroupOffset -= 4;
    p = csu->cacheMem + csu->curKeyOffset;
    *p = 2;
    *(p+1) = keyLen;
    *(unsigned short *)(p+2) = csu->cacheMemTail;
    *(unsigned short *)(csu->cacheMem + csu->curLabelOffset + 2) += 4;
    *(unsigned short *)(csu->cacheMem + csu->curGroupOffset + 2) += 4;
...
PCFG_STRU addMemToCache( const PCFG_STRU csu )
{
    ...
    unsigned short moveLen;

    if ( csu->memSize > 65535 - ATOM_CACHE_SIZE ) return csu;
...
void adjustOffsetInCache( PCFG_STRU csu, long offset, long addOffset )
{
    ...
    while ( p < tail )
    {
	curMem = csu->cacheMem + *(unsigned short *)(p+itemOff[*p]-2);
...
long getSecondLabelOffset( PCFG_STRU csu )
{
    ...
    unsigned short labelMemOff;

    p = csu->cacheMem + csu->curGroupOffset;
    groupOffset = *(long*)(csu->cacheMem + *(unsigned short *)(p+4) + *(p+1) - 4);
    p += 6;

    labelMemOff = *(unsigned short *)(p+2);
    if ( *p == 1 && *(p+1) == 4 &&  labelMemOff != 6 && labelMemOff != 10 ) {
	p += *(unsigned short *)(p+2) - 4;
	curMem =  csu->cacheMem +*(unsigned short *)(p+2) + *(p+1) - 4;
	offset = *(long *)curMem;
	fseek( csu->fp, offset, SEEK_SET );
	while ( !feof( csu->fp ) )  {
	    fgets( buf, 4096, csu->fp );
	...
	curMem = csu->cacheMem + *(unsigned short *)(p+4) + *(p+1) - 4;
	offset = *(long *)curMem;
	if ( groupOffset >= offset ) {
	    fseek( csu->fp, offset, SEEK_SET );
	    fgets( buf, 4096, csu->fp );
				     ...
				     int	   *iBufSize )
{
    ...
    *buf = '\0';
    ...
    while ( p < tail && *p == 0 )
    {
	sz = csu->cacheMem + *(unsigned short *)(p+4);
	if( szGroup == NULL ) {
	...
	*iBufSize = i;
    ...
    while ( p < tail && *p == 2 )
    {
	sz = csu->cacheMem + *(unsigned short *)(p+2);
	if( szKey == NULL ) {
