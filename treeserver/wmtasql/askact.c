...
/*    if( *CurState == 0 ) {
	...
	for( i = tdf->field_num-1;   i >= 0;   i-- ) {
		if( (j = GetFldid(sdf, tdf->field[i].field)) != 0xFFFF ) {
		    if( tdf->field[i].fieldtype == 'M' || tdf->field[i].fieldtype == 'G') {
			  char        buf[260];
			  extern char tmpPath[MAXPATH];
			  short	      newid;
			  dFILE	     *df1;
...
extern WSToMT short  asqlStatMaxY;
	...
	//2001.7.10 Xilong
	if( p->ArrayDim[1] <= *(long *)lpOpnd[0].values )
	{
	    	ErrorSet.xERROR = iStatArrayUseErr;
		sprintf(ErrorSet.string, "[统计列:%d]", *(long *)lpOpnd[0].values+1);
		return  1;
	}
...
extern WSToMT short  asqlStatMaxY;
	...
	if( i < 0 || i >= asqlStatMaxY ) 
	{
	    //统计数组访问越界
	    ErrorSet.xERROR = iErrArrayUse;
	    sprintf(ErrorSet.string, "[统计行:%d]", i);
	    return  1;
	}
...
extern WSToMT short  asqlStatMaxY;
	...
	{
	    	ErrorSet.xERROR = iStatArrayUseErr;
		sprintf(ErrorSet.string, "[求和列:%d]", *(long *)lpOpnd[0].values+1);
		return  1;
	}
...
extern WSToMT short  asqlStatMaxY;
	...
	{
	    	ErrorSet.xERROR = iStatArrayUseErr;
		sprintf(ErrorSet.string, "[求最大列:%d]", *(long *)lpOpnd[0].values+1);
		return  1;
	}
...
extern WSToMT short  asqlStatMaxY;
	...
	{
	    	ErrorSet.xERROR = iStatArrayUseErr;
		sprintf(ErrorSet.string, "[求最小列:%d]", *(long *)lpOpnd[0].values+1);
		return  1;
	}
