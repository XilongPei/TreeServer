     ...
     void (*fatal_func) (char *);
{
  ...
  _gdbm_errno = GDBM_NO_ERROR;

  /* Allocate new info structure. */
  dbf = (gdbm_file_info *) malloc (sizeof (gdbm_file_info));
  if (dbf == NULL)
    {
      _gdbm_errno = GDBM_MALLOC_ERROR;
      return NULL;
    }
  ...
  if (dbf->desc < 0)
    {
      free (dbf->name);
      free (dbf);
      _gdbm_errno = GDBM_FILE_OPEN_ERROR;
      return NULL;
    }
  ...
  if (flags == GDBM_READER)
      ...
      if (file_stat.st_size == 0)
	{
	  ...
	  free (dbf->name);
	  free (dbf);
	  _gdbm_errno = GDBM_EMPTY_DATABASE;
	  return NULL;
	}
  ...
  if (lock_val != 0)
    {
      ...
      free (dbf);
      if (flags == GDBM_READER)
	_gdbm_errno = GDBM_CANT_BE_READER;
      else
	_gdbm_errno = GDBM_CANT_BE_WRITER;
      return NULL;
    }
  ...
  if (file_stat.st_size == 0)
	...
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_BLOCK_SIZE_ERROR;
	  return NULL;
	}
      ...
      if (dbf->bucket == NULL)
	{
      ...
      for (index = 0; (unsigned)index < dbf->header->dir_size / sizeof (off_t); index++)
	...
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_FILE_WRITE_ERROR;
	  return NULL;
	}
	...
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_FILE_READ_ERROR;
	  return NULL;
	}
	...
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_BAD_MAGIC_NUMBER;
	  return NULL;
	}
      ...
      if (dbf->header == NULL)
	{
      ...
      num_bytes = read (dbf->desc, &dbf->header->avail.av_table[1],
			dbf->header->block_size-sizeof (gdbm_file_header));
      if ((unsigned)num_bytes != dbf->header->block_size-sizeof (gdbm_file_header))
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_FILE_READ_ERROR;
	  return NULL;
	...
	{
	  gdbm_close (dbf);
	  _gdbm_errno = GDBM_FILE_SEEK_ERROR;
	  return NULL;
	}
