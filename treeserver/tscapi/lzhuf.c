...
#define  __USE_FOR_TREESVR_CLIENT__
...
#include "terror.h"
#include "tlimits.h"
#ifdef __USE_FOR_TREESVR_CLIENT__
#include "namedpipe.h"
#endif
//#include "tsftp.h"
...
#define F               64      /* Size of look-ahead buffer */
#define THRESHOLD	2
...
typedef struct tagLSZZInfo {
	...
	FILE	*infile, *outfile;
	unsigned long int  textsize, codesize, decodesize;
...
_inline void InsertNode(short r, CLSZZInfo *LSZZInfo);  /* Inserting node to the tree */
_inline void DeleteNode(short p, CLSZZInfo *LSZZInfo);  /* Deleting node from the tree */
...
_inline void EncodeChar(unsigned short c, CLSZZInfo *LSZZInfo);
_inline void EncodePosition(unsigned short c, CLSZZInfo *LSZZInfo);
...
//Initializing tree
///////////////////////////////////////////////////////////////////////////////
void InitTree(CLSZZInfo *LSZZInfo)  
{
	short  i;
...
///////////////////////////////////////////////////////////////////////////////
_inline void InsertNode(short r, CLSZZInfo *LSZZInfo)
{
	short	i, p, cmp;
	unsigned char  *key;
	unsigned short c;
...
_inline void DeleteNode(short p, CLSZZInfo *LSZZInfo)
{
	short  q;
...
/* get one bit */
short GetBit(CLSZZInfo *LSZZInfo)
{
	short i;
	...
	if ( LSZZInfo->Mode == FileMode ){
		...
		while ( LSZZInfo->getlen <= 8 ) {
			if ((i = Memgetc(&(LSZZInfo->inMemInfo))) < 0) 
				i = 0;
			LSZZInfo->getbuf |= i << (8 - LSZZInfo->getlen);
			LSZZInfo->getlen += 8;
		}
...
/* get a byte */
short GetByte(CLSZZInfo *LSZZInfo)	
{
	unsigned short i;

	if ( LSZZInfo->Mode == FileMode ) {
		while (LSZZInfo->getlen <= 8) {
			if ((i = getc(LSZZInfo->infile)) == EOF ) 
				i = 0;
			LSZZInfo->getbuf |= i << (8 - LSZZInfo->getlen);
			LSZZInfo->getlen += 8;
		}
		...
		while (LSZZInfo->getlen <= 8) {
			if ((i = (short)Memgetc(&(LSZZInfo->inMemInfo))) == EOF ) 
				i = 0;
...
int Memgetc(CMemInfo *MemInfo)
{
	...
	if (MemInfo->CurPos < 0 || MemInfo->CurPos == MemInfo->length) {
		...
		dwRetCode = CliReadTSNamedPipe(MemInfo->lpExchBufInfo, buf, &dwBufSize);
#else
		...
		MemInfo->CurPos = 0;
		MemInfo->length = lptsComProps->len;
		memcpy(MemInfo->pMem, (LPVOID)(buf+sizeof(TS_COM_PROPS)), lptsComProps->len);
	    } else {
		return EOF;
	    }
	}
...
int Memputc(int c, CMemInfo *MemInfo)
{
	...
	if (MemInfo->CurPos == MemInfo->length) {
		...
		lptsComProps->len = (short)(MemInfo->length);

		memcpy((LPVOID)(buf+sizeof(TS_COM_PROPS)), MemInfo->pMem, MemInfo->length);
		dwBufSize = sizeof(TS_COM_PROPS) + MemInfo->length;
#ifdef __USE_FOR_TREESVR_CLIENT__
		dwRetCode = CliWriteTSNamedPipe(MemInfo->lpExchBufInfo, buf, &dwBufSize);
#else
		SrvWriteExchngBuf(MemInfo->lpExchBufInfo, buf, dwBufSize);
#endif
		MemInfo->CurPos = 0;
		} else {
		return EOF;
...
size_t Memread(void *buffer, size_t size, size_t count, CMemInfo *MemInfo)
{
	...
	dwBufSize = PIPE_BUFFER_SIZE;
		...
		dwRetCode = CliReadTSNamedPipe(MemInfo->lpExchBufInfo, buf, &dwBufSize);
#else
		dwRetCode = SrvReadExchngBuf(MemInfo->lpExchBufInfo, buf, dwBufSize);
#endif
		if( dwRetCode != TERR_SUCCESS ) {
			MemInfo->fEof = TRUE;
		}
...
//Memwrite() can write the head info (file size) only
size_t Memwrite(const void *buffer, size_t size, size_t count, CMemInfo *MemInfo)
{
	long iwritesize;
...
//compress a file, send out the compressed result to TreeSVR client
//
int FileCompressToSend(LPSTR lpFileName, void *lpExchBufInfo)
{
	...
	if( LSZZInfo.inMemInfo.inFile == NULL ) {
		memset(lptsComProps, 0, sizeof(TS_COM_PROPS));
		lptsComProps->packetType = pkTS_PIPE_FTP;
		lptsComProps->msgType = msgTS_FTP_FILE;
...
#ifdef __USE_FOR_TREESVR_CLIENT__
		dwRetCode = CliWriteTSNamedPipe(lpExchBufInfo, buf, &dwBufSize);
#else
		dwRetCode = SrvWriteExchngBuf( lpExchBufInfo, buf, dwBufSize );
#endif
		return -1;
	}

	
#ifndef __USE_FOR_TREESVR_CLIENT__
	//give other client an information
	////////////////////////////////////////////////////////////////////////////
	memset(lptsComProps, 0, sizeof(TS_COM_PROPS));
	lptsComProps->packetType = pkTS_PIPE_FTP;
	lptsComProps->msgType = msgTS_FTP_FILE;
	lptsComProps->leftPacket = '\x1';
	lptsComProps->endPacket = '\x1';	//I will send you more
	...
	LSZZInfo.inMemInfo.lpExchBufInfo = lpExchBufInfo;
	...
	i = Encode(&LSZZInfo);
	fclose( LSZZInfo.inMemInfo.inFile );
	...
	memset( (LPVOID)lptsComProps, 0, sizeof(TS_COM_PROPS) );
	lptsComProps->len = (short)(LSZZInfo.outMemInfo.CurPos);
	lptsComProps->packetType = pkTS_PIPE_FTP;
	lptsComProps->msgType = msgTS_FTP_FILE;
	lptsComProps->lp = cmFTP_FILE_END;

	memcpy((LPVOID)(buf+sizeof(TS_COM_PROPS)), buff, lptsComProps->len);


	dwBufSize = sizeof(TS_COM_PROPS) + lptsComProps->len;
#ifdef __USE_FOR_TREESVR_CLIENT__
	dwRetCode = CliWriteTSNamedPipe(lpExchBufInfo, buf, &dwBufSize);
#else
	SrvWriteExchngBuf(lpExchBufInfo, buf, dwBufSize);
#endif
...
int FileDecompressFromGet(LPSTR lpFileName,
						void *lpExchBufInfo)
{
	//parameter availability check
	CLSZZInfo LSZZInfo;
	...
	LSZZInfo.outMemInfo.CurPos = -1;
	LSZZInfo.inMemInfo.CurPos = -1;
	LSZZInfo.outMemInfo.length = MAX_PKG_MSG_LEN;
	...
	i = Decode(&LSZZInfo);
	fclose( LSZZInfo.inMemInfo.outFile );
	if( i ) {
		memset( (LPVOID)lptsComProps, 0, sizeof(TS_COM_PROPS) );
		lptsComProps->packetType = pkTS_PIPE_FTP;
		lptsComProps->msgType = msgTS_FTP_FILE;
